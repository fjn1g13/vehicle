...
instance Pretty LeanModule where
  pretty = \case
    OrderDef -> "Order.Def"
...
-- | Inserts arguments to Lean-style notation
...
compileBuiltin b args = case b of
  ...
  where
    ...
    unsupportedError =
      developerError $
        "compilation of builtin" <+> quotePretty b <+> "to Lean unsupported"

    unsupportedArgsError :: (MonadLeanCompile m) => m a
